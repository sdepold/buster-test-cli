...
function serverURL(options) {
    ...
    if (href) {
        ...
        var basicAuthMatch = href.match(/:\/\/(.*)@.*/);

        if(basicAuthMatch) {
            basicAuth = basicAuthMatch[1];
            href = href.replace(basicAuth + '@', '');
        }
    }
...
function createServerClient(options) {
    var urlData     = serverURL(options);
    var port        = urlData.port;
    var hostname    = urlData.hostname;

    delete urlData.port;
    delete urlData.hostname;

    return captureServer.createServerClient(port, hostname, urlData);
}
...
function monitorSessionTimeout(session, callback) {
    return setTimeout(function () {
        session.end();
        callback({
            message: "Timed out waiting for session to load"
        });
    }, 5000);
}
...
var testRun = {
    ...
    endSession: function (sessionClient) {
    ...
    runTests: function (session, callback) {
        var timer = monitorSessionTimeout(session, callback);
    ...
    start: function () {
        var serverClient = createServerClient(this.options);
        serverClient.connect().then(function () {
